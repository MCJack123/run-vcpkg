...
beforeEach(() => {
    ...
    getFileHashMock = jest.spyOn(baseUtil, 'getFileHash').mockImplementation((file: string) => {
        return Promise.resolve([`hash-of-${file}`, file]);
    });
});
...
test('run-vcpkg: basic run no exceptions', async () => {
    // Arrange.
...
test('computeCacheKey(): vcpkg not as a submodule (no commit id user provided and no vcpkg.json found)', async () => {
    ...
    expect(await vcpkgutils.Utils.computeCacheKeys(
        baseUtil, "**/notexistent/vcpkg.json",
        ".",
        "",
...
test('computeCacheKey(): vcpkg not as a submodule (no commit id user provided)', async () => {
    // Arrange.
    const expected: baseutil.KeySet = {
        "primary": "runnerOS=imageos_vcpkgGitCommit=1234_appendedKey=appendedCacheKey_vcpkgJson=hash-of-**/vcpkg.json",
        "restore":
            [
                "runnerOS=imageos_vcpkgGitCommit=1234_appendedKey=appendedCacheKey",
                "runnerOS=imageos_vcpkgGitCommit=1234",
                "runnerOS=imageos"
            ]
    };
...
test('computeCacheKey(): vcpkg as a submodule (no commit id user provided)', async () => {
    // Arrange.
    const expected: baseutil.KeySet = {
        "primary": "runnerOS=imageos_vcpkgGitCommit=5678_appendedKey=appendedCacheKey_vcpkgJson=hash-of-**/vcpkg.json",
        "restore":
            [
                "runnerOS=imageos_vcpkgGitCommit=5678_appendedKey=appendedCacheKey",
...
test('computeCacheKey(): vcpkg as a submodule with provided commit it must trigger a warning', async () => {
    // Arrange.
    const expected: baseutil.KeySet = {
        "primary": "runnerOS=imageos_vcpkgGitCommit=0912_appendedKey=appendedCacheKey_vcpkgJson=hash-of-**/vcpkg.json",
        "restore":
            [
                "runnerOS=imageos_vcpkgGitCommit=0912_appendedKey=appendedCacheKey",
    ...
    const vcpkgCommitIdMock = jest.spyOn(vcpkgutils.Utils, 'getVcpkgCommitId').
        mockImplementationOnce(function (baseUtilLib, path): Promise<[string, boolean]> {
            return Promise.resolve(["0912", false]);
        });
...
test('computeCacheKey(): vcpkg with user provided commit it must not trigger a warning', async () => {
    // Arrange.
    const expected: baseutil.KeySet = {
        "primary": "runnerOS=imageos_vcpkgGitCommit=userId_appendedKey=appendedCacheKey_vcpkgJson=hash-of-**/vcpkg.json",
        "restore": [
            "runnerOS=imageos_vcpkgGitCommit=userId_appendedKey=appendedCacheKey",
